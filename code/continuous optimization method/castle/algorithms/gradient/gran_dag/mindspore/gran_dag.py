...
class GraNDAG(BaseLearner):
    ...
    def learn(self, data, columns=None, **kwargs):
        ...
        self._train(train_data=train_data, test_data=test_data, CI_table=self.CI_table)
    ...
    def _train(self, train_data, test_data, CI_table):
        """
        ...
        for iter_num in tqdm(range(self.iterations), desc='Training Iterations'):
            ...
            h = np.trace(expm_input) - self.input_dim#hloss
            CI_loss = CI_test_Loss(torch.tensor(w_adj), CI_table)
            if args.hard_constraint == True:
                h += args.lambda_CI * CI_loss
            ...
            if iter_num % self.stop_crit_win == 0:
                ...
                if hard_constraint == False:
                    aug_lagrangians_val.append(
                        [iter_num, loss_val.asnumpy().item() + not_nlls[-1] + lambda_CI * CI_loss])#CIloss
                else:
                    aug_lagrangians_val.append(
                        [iter_num, loss_val.asnumpy().item() + not_nlls[-1]])  # CIloss
