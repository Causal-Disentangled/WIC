...
class NotearsNonlinear(BaseLearner):
    ...
    def __init__(self, CI_table, args, lambda1,
                 lambda2,
                 max_iter,
                 h_tol,
                 rho_max,
        ...
        self.args = args
        self.rho, self.alpha, self.h, self.beta = 1.0, 0.0, np.inf, 1.0
    ...
    def dual_ascent_step(self, model, X):
        ...
        while self.rho < self.rho_max:
            ...
            def closure():
                ...
                lambda_CI = 0.2
                X_hat = model(X_torch)
                CI_loss = CI_test_Loss(torch.tensor(W), self.CI_table)
                loss = squared_loss(X_hat, X_torch)
                h_val = model.h_func()
                if hard_constraint == 1:
                    h_val += lambda_CI * CI_loss
