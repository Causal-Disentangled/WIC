...
def dual_ascent_step(model, X, lambda1, lambda2, rho, alpha, h, rho_max, beta,CI_table, args):
    ...
    while rho < rho_max:
        def closure():
            ...
            if args.hard_constraint == True:
                h_val += args.lambda_CI * CI_loss * beta
            else:
                loss += args.lambda_CI * CI_loss * 0.1
...
def notears_nonlinear(model: nn.Module,
    ...
    rho, alpha, h,beta = 1.0, 0.0, np.inf,1.0
    for _ in range(max_iter):
        rho, alpha, h = dual_ascent_step(model, X, lambda1, lambda2,
                                         rho, alpha, h, rho_max, beta, CI_table, args)
...
def main():
    ...
    CI_table = prepare_CI_table0(all_data, args.CI_order, args.significance_level_0, args.significance_level_1)

    np.savetxt('WG_true.csv', WG_true, delimiter=',')
    X1 = X1.squeeze(-1)
    np.savetxt('X.csv', X1, delimiter=',')
    d = X1.shape[1]

    model = NotearsMLP(dims=[d, 10, 1], bias=True)
    W_est = notears_nonlinear(model, X1, CI_table, args, lambda1=0.01, lambda2=0.01)
    assert ut.is_dag(W_est)
    np.savetxt('W_est.csv', W_est, delimiter=',')
    acc = ut.count_accuracy(G_true, W_est != 0)
    print(acc)
